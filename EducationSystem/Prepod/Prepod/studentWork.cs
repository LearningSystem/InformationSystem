        ...
        private void loadTask_Click(object sender, EventArgs e)
        {
            ...
            if (res == DialogResult.OK)
            {
                if (!File.Exists(prepodPath + studentPath + ofd.SafeFileName))
                {
                    ...
                    tasks[1] = numTask;
                    BlackBox frm = new BlackBox(tasks, numStudent, numPrepod, ofd.FileName);
                    frm.Show();

                    //и все что ниже в try надо перенести в код с черным ящиком, когда задача прошла все тесты
                    //------------------------------------------------
                    //try
                    //{
                    //    File.Copy(ofd.FileName, prepodPath + studentPath + ofd.SafeFileName); 
                    //}
