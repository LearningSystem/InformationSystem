    ...
    public partial class DataBlackBox : Form
    {
        ...
        void LoadExercise()
        {
            ...
            using (sconn)
            {
                ...
                SqlCommand scommand = new SqlCommand();
                scommand.Connection = sconn;
                scommand.CommandText = @"SELECT [Название] from [Задача] where [№ преподавателя]='" + numPrepod.ToString() + "'";
                try
                {
                    SqlDataReader dr = scommand.ExecuteReader();
                    while (dr.Read())
        ...
        void ReadTests(string _select)
        {
            ...
            using (sconn)
            {
                ...
                SqlCommand scommand = new SqlCommand();
                scommand.Connection = sconn;
                scommand.CommandText = @"SELECT [Проверка].[№ проверки] from [Проверка],[Задача] where [Название]='" + _select + "' and [Проверка].[№ задачи]=[Задача].[№ задачи]";
                try
                {
                    SqlDataReader dr = scommand.ExecuteReader();
                    int elem = 1;
        ...
        void LoadTextExer(string _select)
        {
            ...
            if (ok==true)
            {
                 ...
                 using (sconn)
                {
                    ...
                    SqlCommand scommand = new SqlCommand();
                    scommand.Connection = sconn;
                    scommand.CommandText = @"SELECT [ССылка] from [Задача] where [Название]='"+_select+"'";
                    try
                    {
                        SqlDataReader dr = scommand.ExecuteReader();
                        while (dr.Read())
        ...
        private void cmBTest_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                sconn = new SqlConnection(connectionString);
                sconn.Open();
                string query = @"SELECT distinct [Входные данные].[Значение] from [Входные данные],[Проверка],[Задача] where [Проверка].[№ задачи]=[Задача].[№ задачи] and [Задача].[Название]='" + cmBExer.SelectedItem.ToString() + "' and [Входные данные].[№ проверки]='" + index[cmBTest.SelectedIndex] + "'";
                ...
                dr.Close();
                //---
                query = @"SELECT distinct [Выходные данные].[Значение] from [Выходные данные],[Проверка],[Задача] where [Проверка].[№ задачи]=[Задача].[№ задачи] and [Задача].[Название]='" + cmBExer.SelectedItem.ToString() + "' and [Выходные данные].[№ проверки]='" + index[cmBTest.SelectedIndex] + "'";
        ...
        private void удалениеДанныхToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (MessageBox.Show(quest,"Предупреждение",MessageBoxButtons.YesNo)==DialogResult.Yes)
            {
                ...
                using (sconn)
                {
                    ...
                    SqlCommand scommand = new SqlCommand();
                    scommand.Connection = sconn;
                    scommand.CommandText = @"Select [№ задачи] from [Задача] where [Название]='" + cmBExer.SelectedItem.ToString() + "'";
                    SqlDataReader dr = scommand.ExecuteReader();
                    while(dr.Read())
                    {
                        num = dr[0].ToString();
                    } dr.Close();
                    scommand.CommandText = @"Delete from [Проверка] where [Проверка].[№ задачи]='" + num + "' and [№ проверки]='" + index[cmBTest.SelectedIndex] + "'";
                    try
                    {
                        scommand.ExecuteNonQuery();
                    }
